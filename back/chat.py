...
import back.services.classifier_service as classifier_serv
import back.services.escalation_service as escalation_serv
import uuid
...
async def handle_message(message: str, response: Response, session_id: Optional[str] = Cookie(None)):
    ...
    intent = await classifier_serv.intent_classifier(text)

    conversation = dynamo_serv.get_conversation(session_id)
    escalate, reason = escalation_serv.should_escalate(intent, conversation)